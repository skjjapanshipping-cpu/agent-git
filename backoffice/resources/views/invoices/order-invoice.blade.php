...
<body>
    <table class="header-table">
        <tr>
            <td class="logo-cell">
                {{-- <a href="{{ public_path('img/logopdf.png') }}"> --}}
            ...
            <td class="invoice-details-cell">
                ...
                <p>เลขที่: {{ strtoupper($customerno) }}-{{ date('dmY', strtotime($order_date_Original)) }}</p>
            </td>
        </tr>
    </table>
    {{-- <div class="clear"></div> --}}
    ...
    <table>
        <thead >
            <tr>
                ...
                <th width="15%">เงินเยน</th>
                <th width="10%">เรท</th>
                <th width="15%">เงินบาท</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td colspan="6">ค่าสั่งซื้อสินค้า (ระหว่างวันที่ {{ date('d/m/y', strtotime($order_date_Original)) }} - {{ date('d/m/y', strtotime($end_order_date_Original)) }})</td>
         
            </tr>
            @php $total = 0; $totalWeight = 0; @endphp
            ...
            <tr>
                ...
                <td class="text-center">
                    @if($order->imageExists && $order->imagePath)
                        <img src="{{ $order->imagePath }}" width="35" height="35" alt="">
                    @elseif(!empty($order->image_link))
                        <span style="color: red; font-size: 10pt;">ไม่พบไฟล์รูป</span>
                    @else
                        <span style="color: #999; font-size: 10pt;">ไม่มีรูป</span>
                    @endif
                </td>
                <td class="text-center"> 
                    <a href="{{ $order->link }}" target="_blank" style="color: blue; text-decoration: none; font-size: 14pt;">
                        {{ $order->displayLink }}
                    </a>
                </td>
                <td class="text-right">{{ number_format($order->product_cost_yen, 2) }}</td>
                <td class="text-right">{{ number_format($order->rateprice, 3) }}</td>
                <td class="text-right">{{ number_format($order->product_cost_baht, 2) }}</td>
            </tr>
            @php 
                // $total += ($shipping->weight * $shipping->unit_price) + ($shipping->cod ?? 0);
                $total += $order->product_cost_baht;
        ...
        <tfoot>
            <tr >
                ...
                @endphp
                <td colspan="5" style="font-size: 16pt;" class="text-center border-top border-bottom bg-black text-white">{{ convertNumberToThaiBaht($total) }}</td>
    ...
    <div class="signature">
        <div style="float: left; width: 53%; text-align: center;">
            <p style="line-height: 7.0;">รายการสั่งซื้อทั้งหมดชำระเงินเรียบร้อยแล้ว</p>
            <p>.................................... วันที่ ..../..../.....</p>
